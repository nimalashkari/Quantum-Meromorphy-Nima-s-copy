...
We said that compact operators have discrete spectra with a potential essential spectrum $\{0\}$ which if it exists must be an accumulation point. Since every other point in the spectrum is discrete, it has finite multiplicity, so the operator $X-z \mathbb{I}$ for $z>0$ can be inverted up to an obstruction by a finite rank matrix.
...
\(z\in\mathbb C\setminus\{0\}\), the operator $X-z \mathbb{I}$
...
\mathbb C\setminus\{0\},
\]
with finite-rank principal parts at all poles.
\end{enumerate}
\end{theorem}
Note that since the spectrum of self-adjoint operators is inside the real numbers, the theorem above remains correct if we restrict $z\in\mathbb{R}\setminus \{0\}$. A consequence of this theorem is that $(X-z I)^{-1}$ is a meromorphic function 
