...
The statement above connects the Fredholm property to compactness for self-adjoint operators. More generally, for a non-self-adjoint operator, the relation between Fredholmness and compactness is more complicated. In the following, we will need the following theorem:

\begin{theorem}[Finite-core Fredholm criterion]
Let \(U\subset\mathbb C\) be open, and let \(T(z)\) be a holomorphic family of
closed operators on a Hilbert space \(\mathcal H\). Suppose that
\[
\mathcal H=\mathcal H_{\mathrm c}\oplus\mathcal H_{\mathrm t},
\qquad
\dim \mathcal H_{\mathrm c}<\infty ,
\]
and that, with respect to this decomposition,
\[
T(z)=
\begin{pmatrix}
A(z) & B(z)\\
...
\end{pmatrix}.
\]
Assume that the tail block \(D(z):\mathcal H_{\mathrm t}\to\mathcal H_{\mathrm t}\)
is boundedly invertible for every \(z\in U\), and that \(D(z)^{-1}\) is
holomorphic in \(z\). Then \(T(z)\) is Fredholm of index \(0\) for every
\(z\in U\).

Moreover, if \(T(z)\) is invertible for at least one point \(z_0\in U\), then
\[
T(z)^{-1}
\]
is meromorphic on \(U\), with finite-rank principal parts at its poles.
\end{theorem}
...
For non-self-adjoint operators, the spectrum as a set is not enough. A simple
example is an infinite direct sum of Jordan blocks. Let \(\mathcal H=\ell^2\),
and define \(J e_{2k-1}=e_{2k}\) and \(J e_{2k}=0\). Then \(J^2=0\), so
\(\sigma(J)=\{0\}\). For \(z\neq 0\),
\[
...
The correct replacement is Fredholm theory. Instead of asking whether the
