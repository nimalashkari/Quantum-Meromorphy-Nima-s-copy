...
So, for self-adjoint operators, the essential spectrum contains:
\[
\text{continuous spectrum},
\]
\[
\text{eigenvalues of infinite multiplicity},
\]
and
\[
\text{eigenvalues that are not isolated, such as embedded eigenvalues}.
\]
...
subspace, and the associated spectrum is denoted
\[
\sigma_{\mathrm{ac}}(A).
\]

The remaining continuous part is the singular continuous spectrum,
\[
\sigma_{\mathrm{sc}}(A).
\]
...
not the ordinary continuum one encounters in elementary scattering theory.

For ordinary Schrödinger scattering, the absolutely continuous spectrum is the
scattering spectrum. For example, the free Hamiltonian
\[
...
\sigma(H_0)=[0,\infty),
\]
and this spectrum is absolutely continuous. After Fourier transform,
\[
H_0
\]
...
\widehat{H_0\psi}(k)
=
\frac{\hbar^2|k|^2}{2m}\widehat\psi(k).
\]
Thus energy is labelled continuously by the momentum \(k\). The formal
eigenstates are plane waves
\[
e^{ik\cdot x},
\]
which are not normalizable, but wave packets built from them propagate to
spatial infinity. This is the basic meaning of a scattering state.

More generally, for
\[
H=H_0+V(x),
\]
with \(V\) sufficiently localized, the absolutely continuous subspace describes
states that asymptotically behave like free wave packets at large positive and
negative times. In this sense,
\[
\sigma_{\mathrm{ac}}(H)
\]
is the part of the spectrum associated with transport to infinity.

Thus the useful physics dictionary is
\[
\text{point spectrum}
\quad\leftrightarrow\quad
\text{normalizable bound states},
\]
\[
\text{absolutely continuous spectrum}
\quad\leftrightarrow\quad
...
\quad\leftrightarrow\quad
\text{exotic continuous spectral behavior}.
\]
For many standard short-range quantum Hamiltonians, the singular continuous
...
Hilbert space \(\mathcal H\). Then \(X\) is compact if and only if, for every
\(\lambda\in\mathbb C\setminus\{0\}\), the operator
\[
I-\lambda X
\]
...
restriction to real \(z\) is a compact self-adjoint family.
...
\begin{proposition}[Analytic Fredholm theorem]
...
I-\lambda K(z)
\]
is invertible for at least one \(z\in D\).

Then the following are equivalent:

\[
K(z)\text{ is compact for every }z\in D,
\]
...
at least one point \(z_0\in D\), then
\[
T(z)^{-1}
\]
...
T_0(z)^{-1}K(z)
\]
is holomorphic and compact. Hence
\[
I+T_0(z)^{-1}K(z)
\]
